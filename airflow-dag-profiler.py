...
def airflow_dag_profiler(dag_id, debug, date):
    ...
    for bq_task in tqdm(bq_tasks):
        sql = _system(f"airflow render {dag_id} {bq_task} {date.strftime('%Y-%m-%d')}").output
        lines = sql.split("\n")
        ...
        total_bytes_processed = client.query(sql, job_config=bigquery.job.QueryJobConfig(dry_run=True, use_query_cache=False)).total_bytes_processed
        quota.append({"bq_task":bq_task, "total_bytes_processed":total_bytes_processed})
